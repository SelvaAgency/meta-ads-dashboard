...
export default function Admin() {
  const { user } = useAuth();
  /**
   * A regra vem de `canAccessAdmin`, e não de `role !== "admin"` escrito aqui.
   * Comportamento idêntico — o que muda é haver UM lugar onde "quem é admin" se
   * decide, em vez de um por página.
   */
  if (!canAccessAdmin((user as { role?: string } | null)?.role)) return null;
