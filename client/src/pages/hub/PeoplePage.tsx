...
import { Users, Plus, KeyRound, Pencil, Check, X, Loader2, Copy } from "lucide-react";
...
function PersonRow({ p, onTempPassword }: { p: Person; onTempPassword: (email: string, pwd: string) => void }) {
  ...
  const update = trpc.people.update.useMutation({ onSuccess: () => { utils.people.list.invalidate(); setEditing(false); } });
  ...
  return (
    <div className="rounded-lg border border-border p-3 flex flex-wrap items-center gap-3">
