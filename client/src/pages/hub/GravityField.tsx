...
const SLEEP_V2=.09, REST_FRAMES=26, MINOV=.22;
...
export const GravityField = memo(function GravityField({ fill = false, active = true }: { fill?: boolean; active?: boolean } = {}) {
  ...
  useEffect(() => {
    ...
    function collide(a: Ball, b: Ball) {
      ...
      // Sem quique em contato lento (assentamento): restituição 0 → o impulso só
      // CANCELA a aproximação, não devolve energia. Era o quique que mantinha a
      // pilha viva pra sempre. Só batidas rápidas (rv forte) quicam de verdade.
      const e = -rv < .8 ? 0 : REST;
      const j = -(1+e)*rv*.5;
      ...
      if (-rv > .8) { a.still=0; b.still=0; } // impacto real acorda as duas
    }
