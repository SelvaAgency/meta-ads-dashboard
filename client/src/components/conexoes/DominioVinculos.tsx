/**
 * ─────────────────────────────────────────────────────────────────────────────
 *  Domínio do site — configuração por cliente
 * ─────────────────────────────────────────────────────────────────────────────
 *  Esta conexão já foi "Conectar Clarity", e o nome mentia sobre o que ela faz.
 *  O que o cliente entrega aqui é o DOMÍNIO — e é ele que destrava as leituras
 *  técnicas do site (segurança, disponibilidade, carregamento/LiteSpeed) mesmo
 *  quando não existe Clarity nenhum. O Clarity é uma das leituras que o domínio
 *  habilita, não o motivo do cadastro.
 *
 *  Nada foi removido: o token, o projectId e o sync do Clarity continuam
 *  exatamente como estavam. Só o conceito foi reposicionado.
 *
...
function FormCliente({ accountId }: { accountId: number }) {
  ...
  const salvar = trpc.clarity.upsert.useMutation({
    onSuccess: () => { utils.clarity.settings.invalidate(); utils.fontes.todas.invalidate(); toast.success("Domínio salvo."); },
  ...
  const temDominio = !!atual?.domain;
  const clarityAtivo = !!atual?.enabled && !!atual?.hasToken;
    ...
    <div className="bg-card border border-border rounded-xl p-5 flex flex-col gap-3">
      <div className="flex items-center justify-between gap-3 flex-wrap">
        <p className="text-xs text-muted-foreground">
          {temDominio
            ? <>Domínio <span className="text-foreground font-medium">{atual?.domain}</span>{clarityAtivo ? " · Clarity ativo" : ""}{atual?.lastSyncStatus === "erro" ? " · última sincronização do Clarity falhou" : ""}.</>
            : "Nenhum domínio cadastrado para este cliente."}
        </p>
        <div className="flex items-center gap-2">
          ...
          {clarityAtivo && (
            <button onClick={() => sync.mutate({ accountId, dias: 1 })} disabled={sync.isPending}
              ...
              {sync.isPending ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <RefreshCw className="w-3.5 h-3.5" />} Sincronizar Clarity
            </button>
          )}
        </div>
      </div>

      {/* O domínio vem primeiro porque é ele que destrava o resto — sem ele não
          há segurança, disponibilidade nem carregamento para medir. */}
      <Campo label="Domínio do site" hint="Só o domínio, sem https:// — é o que habilita segurança, disponibilidade e leituras de carregamento.">
      ...
      {/* Clarity — uma das leituras que o domínio habilita, e opcional. */}
      <div className="border-t border-border pt-3 flex flex-col gap-3">
        <div>
          <p className="text-xs font-semibold text-foreground">Microsoft Clarity <span className="font-normal text-muted-foreground">· opcional</span></p>
          <p className="text-[11px] text-muted-foreground mt-0.5">
            Comportamento das pessoas no site (sessões, scroll, cliques mortos). Sem isto, o domínio ainda
            responde pelas leituras técnicas.
          </p>
        </div>
        ...
        <Campo label="Token da API do Clarity" hint={atual?.hasToken ? "Já existe um token salvo. Preencha só para substituir." : "Clarity → Settings → Data Export → Generate new API token."}>
          <input type="password" value={apiToken} onChange={(e) => setApiToken(e.target.value)}
            placeholder={atual?.hasToken ? "•••••••• (mantém o atual)" : "Cole o token aqui"} className={inp} />
        </Campo>

        <Campo label="Project ID do Clarity" hint="Só para o link do painel. A API não usa — quem identifica o projeto é o token.">
...
export function DominioVinculos() {
  const { user } = useAuth();
  ...
  return (
    <div className="flex flex-col gap-3">
      <div>
        <h3 className="text-sm font-bold text-foreground flex items-center gap-2"><Globe className="w-4 h-4" /> Domínio do site · por cliente</h3>
        <p className="text-xs text-muted-foreground mt-0.5">
          Adicione o domínio do site para habilitar leituras técnicas, Clarity e performance. Os dados
          aparecem na seção Site do cliente.
        </p>
      </div>
