...
// `prev` (valor bruto do período anterior, já formatado) aparece no hover — o
// mesmo padrão do resto do dashboard.
export function VariacaoBadge({ v, prev }: { v: Variacao; prev?: string }) {
  if (!v) return null;
  const cor = v.sobe ? "text-emerald-600 dark:text-emerald-400" : "text-red-500 dark:text-red-400";
  const badge = <span className={`text-[11px] font-bold ${cor} flex-shrink-0`}>{v.sobe ? "▲" : "▼"} {Math.abs(v.pct).toFixed(0)}%</span>;
...
export function KpiTile({ rotulo, valor, variacao, prevValor }: { rotulo: string; valor: ReactNode; variacao?: Variacao; prevValor?: string }) {
  return (
    <div className="rounded-xl border border-border bg-card p-3">
      <div className="flex items-start justify-between gap-1.5">
        <p className="text-[10px] uppercase tracking-wide text-muted-foreground font-semibold truncate">{rotulo}</p>
        {variacao && <VariacaoBadge v={variacao} prev={prevValor} />}
      </div>
      <p className="text-xl font-extrabold text-foreground leading-tight mt-0.5">{valor}</p>
    </div>
  );
}
