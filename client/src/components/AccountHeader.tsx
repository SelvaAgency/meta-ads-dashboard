...
{erroSync}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 px-4 pb-4">
        ...
        <div className="rounded-xl border border-border bg-background/30 p-4 flex flex-col"
          ...
          {/* O ponto técnico NÃO aparece aqui.

              Resumo = CONCLUSÃO. Ponto técnico = problema + contexto + ação.
              Misturar os dois fazia o cabeçalho repetir, em linguagem de
              alerta, o que a frase acima já tinha acabado de concluir — e ainda
              expor "Contextualizado", que é vocabulário interno do sistema e
              não diz nada a quem lê.

              O alerta e a ação de explicá-lo vivem no Panorama, junto dos
              outros achados, onde cabem problema, explicação e ação na mesma
              linha. A conclusão do resumo já reflete o contexto, porque a IA o
              recebe com autoridade — e é isso que o cabeçalho precisa mostrar. */}
          {summaryOverflows && (
      ...
      {contextOpen && selectedAccountId && (
        <div className="px-4 pb-4">
          <ContextoGeralPanel
            ...
            metasSlot={<ThresholdsPanel account={{ id: selectedAccountId, goalTypeOverride: goalType }} />}
          />
        </div>
      )}
    </div>
