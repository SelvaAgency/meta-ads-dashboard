/**
 * ─────────────────────────────────────────────────────────────────────────────
 *  O painel contextual dos cliques no link
 * ─────────────────────────────────────────────────────────────────────────────
 *  Um cartão permanente para cliques no link seria desproporcional: é o número
 *  menor da faixa, e ainda assim ocuparia a mesma área que engajamento. Mas a
 *  pergunta "os cliques estão subindo?" é legítima e não cabe num número só.
 *
 ...
 *
 *  ── A relação com seguidores usa a taxa que já existe ─────────────────────
 ...
 * coleta não autoriza.
 */
function MiniSerie({ dias, cor, altura = 46 }: {
  dias: DiaDeCliques[]; cor: string; altura?: number;
}) {
  const medidos = dias.filter((d) => d.cliques != null);
  if (medidos.length < 2) {
    return (
  ...
  const max = Math.max(1, ...medidos.map((d) => d.cliques as number));
  ...
  dias.forEach((d, i) => {
    ...
    atual.push(`${atual.length ? "L" : "M"}${x(i).toFixed(1)},${y(d.cliques).toFixed(1)}`);
  });
  if (atual.length > 1) partes.push(atual.join(" "));

  const ultimo = dias.reduce((achado, d, i) => (d.cliques != null ? i : achado), -1);

  return (
    <svg viewBox={`0 0 ${W} ${altura}`} width="100%" height={altura} role="img"
      aria-label="Evolução dos cliques no link no período">
      ...
      {ultimo >= 0 && (
        <circle cx={x(ultimo)} cy={y(dias[ultimo].cliques as number)} r={2.6} fill={cor} />
      )}
    </svg>
  );
}

export function PainelDeCliques({ dias, total, variacaoPct, anterior, motivoSemComparacao,
  seguidores, children }: {
  dias: DiaDeCliques[];
  total: number | null;
  ...
  const porSeguidores = taxaPorSeguidores(total, seguidores);
  const medidos = dias.filter((d) => d.cliques != null).length;
    ...
    <Popover>
      ...
      <PopoverContent align="start" sideOffset={10} className="w-[292px] p-4 rounded-[14px]">
        ...
        <span className="block text-[26px] font-bold tabular-nums leading-none tracking-tight mt-2"
          style={{ color: COR.visitas }}>
          {fmt(total)}
        </span>

        <div className="mt-3">
          <MiniSerie dias={dias} cor={COR.visitas} />
        </div>

        <dl className="grid grid-cols-2 gap-x-3 gap-y-2.5 mt-3 pt-3 border-t border-border">
          <div>
        ...
        <p className="text-[9.5px] text-muted-foreground/55 leading-snug mt-3">
          Cliques no link da bio, medidos por <span className="font-mono">website_clicks</span>.
          Não inclui cliques em links de story.
        </p>
      </PopoverContent>
    </Popover>
  );
