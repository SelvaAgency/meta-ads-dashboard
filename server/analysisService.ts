...
- metric: a métrica principal que vai mudar como RESULTADO da ação — sempre da CONTA OU CAMPANHA, nunca do elemento sendo pausado. Ex: se pausa um conjunto com CPA alto, monitore o CPA da campanha inteira. Se pausa um criativo com frequência alta, monitore o ROAS ou CPA da campanha.
- baseline: o valor ATUAL dessa métrica (da conta/campanha) extraído dos dados fornecidos (número real)
- target: o valor esperado após a ação — NUNCA use 0 como target. Use benchmarks internos, média da conta, ou melhor performer como referência. Ex: se CPA médio da conta é R$303 e a ação deve aproximar o resultado desse valor, target = 303.
- direction: "decrease" se menor é melhor (CPA, frequência, CPC, spend em conjuntos improdutivos), "increase" se maior é melhor (ROAS, CTR, conversões)
- unit: "BRL" para valores em reais, "x" para ROAS, "%" para taxas, "" para contagens
- description: uma frase curta em português explicando o impacto esperado na conta/campanha (ex: "Redução do CPA da campanha de R$400 para próximo da média da conta R$303")
